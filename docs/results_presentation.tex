...
\normalsize Simulation \& GNU Radio Software Loopback Results}
...
\begin{center}
  % Video embedded in PDF — VPlayer.swf plays MP4 inline in Adobe Acrobat Reader
  \includemedia[
    width=0.80\textwidth,
    height=0.45\textwidth,
    activate=onclick,
    addresource=grc_screen_record.mp4,
    addresource=VPlayer.swf,
    flashvars={
      src=grc_screen_record.mp4
      &autoPlay=false
      &loop=false
    },
    passcontext
  ]{\includegraphics[width=0.80\textwidth]{grc_poster_frame.png}}{VPlayer.swf}
  \\[0.3em]
  {\small\textbf{Click image to play inline}\enspace{\scriptsize(Adobe Acrobat Reader)}}\\[0.15em]
  \href{https://nusu-my.sharepoint.com/:v:/g/personal/manujayaugp_u_nus_edu/IgBL8WLq6Kg3T72oFts44raeAY2qRBnLZsHWS2kot5pOpYc}{%
    \textcolor{NUSorange}{\large$\blacktriangleright$}\enspace
    \textbf{\textcolor{NUSorange}{Or watch full-screen on NUS OneDrive}}}\\[0.1em]
  {\scriptsize\textcolor{NUSgray}{(Edge, Chrome, phone — any browser)}}
\end{center}
\end{frame}

% ── GNU Radio Demo — Content slide ───────────────────────────
\begin{frame}[shrink=18]{GNU Radio Software Loopback Demo — What It Shows}
\begin{columns}[T]
  \begin{column}{0.54\textwidth}
    \textbf{Current setup:}\\
    Random source $\to$ full IQ signal chain $\to$ AWGN channel (no USRP yet)
    ...
    \begin{enumerate}
      ...
      \item AWGN noise increases — MODCOD steps down\\
            (32APSK $\to$ 16APSK $\to$ 8PSK $\to$ QPSK)
      \item Noise reduces — link recovers to high-order MODCOD
    \end{enumerate}
  \end{column}
  \begin{column}{0.43\textwidth}
    \begin{keybox}{Key point}
      Full BCH+LDPC FEC, real Pilot-MMSE SNR estimation, and PLSCODE signalling — all working in software loopback. Constellation changes \textit{automatically} with no manual intervention.
    \end{keybox}

    \vspace{0.6em}
    \begin{resultbox}{Next step}
      Connect USRP B200 hardware and run \texttt{acm\_loopback.grc} over a real X-Band link for hardware validation.
    \end{resultbox}
  \end{column}
\end{columns}
\end{frame}
