...
\end{columns}
\end{frame}
% ── GNU Radio Demo — Embedded Video ──────────────────────────
\begin{frame}[shrink=4]{GNU Radio Software Loopback Demo}
\begin{columns}[T]
  \begin{column}{0.56\textwidth}
    \begin{center}
      \movie[width=\linewidth, height=0.5625\linewidth,
             poster, showcontrols]{%
        \includegraphics[width=\linewidth]{grc_poster_frame.png}%
      }{grc_screen_record.mp4}\\[0.2em]
      {\tiny\textcolor{NUSgray}{\textbf{Option 1:} Click above (Acrobat Reader)\quad
        \textbf{Option 2:}\enskip\href{https://nusu-my.sharepoint.com/:v:/g/personal/manujayaugp_u_nus_edu/IgBL8WLq6Kg3T72oFts44raeAY2qRBnLZsHWS2kot5pOpYc}{\textcolor{NUSorange}{\textbf{$\blacktriangleright$ OneDrive full-screen}}}}}
    \end{center}
  \end{column}
  \begin{column}{0.41\textwidth}
    \small\textbf{Setup:} Random source $\to$ full IQ signal chain $\to$ AWGN channel
    \vspace{0.3em}

    \textbf{What the video shows:}
    \begin{enumerate}\small
      \item GRC flowgraph — TX, channel, RX, ACM controller
      \item Execution starts — QPSK constellation appears
      \item Noise increases — MODCOD steps down\\(32APSK $\to$ 16APSK $\to$ 8PSK $\to$ QPSK)
      \item Noise reduces — link recovers to high-order MODCOD
    \end{enumerate}
    \vspace{0.3em}
    \begin{keybox}{Key point}
      \scriptsize Full BCH+LDPC FEC, Pilot-MMSE SNR estimation, PLSCODE signalling — all working. Constellation changes \textit{automatically}.
    \end{keybox}
  \end{column}
\end{columns}
\end{frame}
