%% ============================================================
%% DVB-S2 ACM — Simulation & Software Loopback Results
...
\normalsize Simulation \& Software Validation Results}
...
\begin{center}
\small
\textcolor{NUSgray}{LEO X-Band Link $\cdot$ 500 km $\cdot$ 8.025 GHz $\cdot$ GNU Radio 3.10 Software Loopback}
\end{center}
\end{frame}
...
\begin{columns}[T]
  ...
  \begin{column}{0.48\textwidth}
    ...
    \begin{resultbox}{Use when}
      \small Validating the signal chain, demonstrating the live system, and preparing for USRP B200 hardware deployment.
    \end{resultbox}
  \end{column}
\end{columns}
\vspace{0.4em}
...
\begin{frame}[shrink=10,fragile]{AI/ML Engine: Dueling Double DQN}
\begin{columns}[T]
...
\begin{frame}[shrink=20]{Cross-Scenario Comparison Summary}
\begin{center}
\renewcommand{\arraystretch}{1.1}
\small
\begin{tabular}{llccc}
  \toprule
  \textbf{Scenario} & \textbf{Strategy} & \textbf{$\eta$ (b/s/Hz)} &
  \textbf{Switches} & \textbf{QEF\%} \\
  \midrule
  \multirow{3}{*}{SNR Sweep}
    & CCM             & 0.99 & 0   & 76.0 \\
    & Rule-based      & \textbf{2.08} & 42  & 47.4 \\
    & \textbf{DQN}   & 1.43 & 56 & \textbf{70.5} \\
  \midrule
  \multirow{3}{*}{LEO Pass}
    & CCM             & 0.99 & 0   & 99.8 \\
    & Rule-based      & \textbf{3.02} & 410 & 65.0 \\
    & \textbf{DQN}   & 1.77 & 891 & \textbf{91.6} \\
  \midrule
  \multirow{3}{*}{Rain Fade}
    & CCM             & 0.99 & 0  & 100.0 \\
    & Rule-based      & \textbf{3.06} & 20 & 67.2 \\
    & \textbf{DQN}   & 1.78 & 87 & \textbf{93.8} \\
  \bottomrule
\end{tabular}
\end{center}
\vspace{-0.3em}
  ...
  \begin{column}{0.48\textwidth}
    \begin{resultbox}{DQN strengths}
      ...
      \begin{itemize}\setlength{\itemsep}{0pt}
        \item Best QEF\% in 2/3 scenarios
        \item Conservative during fades (reward penalises FER)
      \end{itemize}
    \end{resultbox}
  \end{column}
  \begin{column}{0.48\textwidth}
    \begin{notebox}{Trade-off}
      \scriptsize
      \begin{itemize}\setlength{\itemsep}{0pt}
        \item DQN conservatively trades $\eta$ for QEF reliability
        \item 0.29\,ms/decision — real-time feasible (4.4\% frame)
      \end{itemize}
    \end{notebox}
  \end{column}
\end{columns}
...
\end{columns}
\end{frame}
% ── Simulation Demo — Video slide ────────────────────────────
\begin{frame}[shrink=8]{Python Simulation Demo — acm\_loopback\_sim.py}
\begin{center}
  \includemedia[
    width=0.80\textwidth,
    height=0.45\textwidth,
    activate=onclick,
    addresource=sim_screen_record.mp4,
    passcontext
  ]{\href{https://nusu-my.sharepoint.com/:v:/g/personal/manujayaugp_u_nus_edu/IQC4RPBsyLUYTKmggNV0iH9TAYb82JL_AcG6a3O7OpCzZs8?nav=eyJyZWZlcnJhbEluZm8iOnsicmVmZXJyYWxBcHAiOiJPbmVEcml2ZUZvckJ1c2luZXNzIiwicmVmZXJyYWxBcHBQbGF0Zm9ybSI6IldlYiIsInJlZmVycmFsTW9kZSI6InZpZXciLCJyZWZlcnJhbFZpZXciOiJNeUZpbGVzTGlua0NvcHkifX0&e=P2MvmI}{\includegraphics[width=0.80\textwidth]{sim_poster_frame.png}}}{sim_screen_record.mp4}
  \\[0.3em]
  {\small\textbf{Click image to play} {\scriptsize(Adobe Acrobat)} \textbf{or open video link} {\scriptsize(any viewer)}}
\end{center}
\end{frame}

% ── GNU Radio Demo — Video slide ─────────────────────────────
\begin{frame}[shrink=8]{GNU Radio Software Loopback Demo}
\begin{center}
  \includemedia[
    width=0.80\textwidth,
    height=0.45\textwidth,
    activate=onclick,
    addresource=grc_screen_record.mp4,
    passcontext
  ]{\href{https://nusu-my.sharepoint.com/:v:/g/personal/manujayaugp_u_nus_edu/IgBL8WLq6Kg3T72oFts44raeAY2qRBnLZsHWS2kot5pOpYc}{\includegraphics[width=0.80\textwidth]{grc_poster_frame.png}}}{grc_screen_record.mp4}
  \\[0.3em]
  {\small\textbf{Click image to play} {\scriptsize(Adobe Acrobat)} \textbf{or open video link} {\scriptsize(any viewer)}}
\end{center}
\end{frame}
...
\begin{columns}[T]
  ...
  \begin{column}{0.42\textwidth}
    \begin{resultbox}{Best result}
      ...
      $\eta = 1.78$ bits/sym with QEF $= 93.8\%$\\
      vs rule-based QEF $= 67.2\%$\\[0.3em]
      \textbf{+27 percentage points} reliability\\with $1.8\times$ CCM throughput.
    \end{resultbox}

    \vspace{0.5em}
    \begin{keybox}{Next step}
      \small Connect USRP B200 hardware and run \texttt{acm\_loopback.grc} over a real X-Band link for hardware validation.
    \end{keybox}
  \end{column}
\end{columns}
\end{frame}
